...
write_data_front_to_back(channel * curr_channel)
{
    ...
    if (bytes_written == -1)
    {
        log_write(ERROR, "error while writing from front to back");
        return -1;
    }
    return 0;
}
...
write_data_back_to_front(channel * curr_channel)
{
    ...
    if (bytes_written == -1)
    {
        log_write(ERROR, "error while writing from back to front");
        return -1;
    }
    return 0;
}
...
connect_postgres_server()
{
    ...
    log_write(INFO, "proxy has connected to postgres server successfully\n");
